...
{                               /* --- create an item set reporter */
  ...
  k = (max > n) ? max : n;      /* get maximum item set size */
...
int isr_extrule (ISREPORT *rep,
                 ...
                 RSUPP body, RSUPP sa, RSUPP sb, RSUPP sab)
{                               /* --- report special seq. rule */
  ITEM i;                       /* buffer for the item counter */

  assert(rep                    /* check the function arguments */
  &&    (!items || (n > 0)) && (a >= 0) && (b >= 0)
  &&    (body > 0) && (sa > 0) && (sb > 0) && (sab >= 0));
  if (!items) {                 /* if no items given, use internal */
    items = rep->items; n = rep->cnt; }
  if ((body < rep->smin) || (body > rep->smax))
    return 0;                   /* check the item set support */
  if ((n +2 < rep->zmin) || (n +2 > rep->zmax))
    return 0;                   /* check the item set size */
  rep->stats[n+2] += 1;         /* count the reported rule */
  rep->repcnt     += 1;         /* (for its size and overall) */
  if (!rep->file) return 0;     /* check for an output file */
  i = rep->cnt; rep->cnt = n+2; /* note the number of items */
  isr_puts(rep, rep->hdr);      /* print the record header */
  if (--n >= 0)                 /* print the first item in body */
    isr_puts(rep, rep->inames[*items++]);
  while (--n >= 0) {            /* traverse the remaining items */
...
int isr_xinfo (ISREPORT *rep, RSUPP body, RSUPP sa, RSUPP sb, RSUPP sab)
{                               /* --- print ext. rule information */
  int        k, n = 0;          /* number of decimals, char. counter */
  double     smax, supp;        /* (maximum) support */
  double     conf;              /* buffers for computations */
  const char *s, *t;            /* to traverse the format */
  ...
  for (s = rep->info; *s; ) {   /* traverse the output format */
    ...
    switch (*s++) {             /* evaluate the indicator character */
      ...
      #if RSUPP==double
      case 'b': n += isr_numout(rep,             body,        k); break;
      case 'n': n += isr_numout(rep,             sa,          k); break;
      case 'm': n += isr_numout(rep,             sb,          k); break;
      case 'a': n += isr_numout(rep,             sab,         k); break;
      case 'A': n += isr_numout(rep,             sa-sab,      k); break;
      case 'B': n += isr_numout(rep,             sb-sab,      k); break;
      #else
      case 'b': n += isr_intout(rep, (ptrdiff_t) body);           break;
      case 'n': n += isr_intout(rep, (ptrdiff_t) sa);             break;
      case 'm': n += isr_intout(rep, (ptrdiff_t) sb);             break;
      case 'a': n += isr_intout(rep, (ptrdiff_t) sab);            break;
      case 'A': n += isr_intout(rep, (ptrdiff_t)(sa-sab));        break;
      case 'B': n += isr_intout(rep, (ptrdiff_t)(sb-sab));        break;
      #endif
      #undef int
      #undef double
      case 'x': n += isr_numout(rep,      (double)body/smax,  k); break;
      case 'X': n += isr_numout(rep, 100*((double)body/smax), k); break;
      case 'y': n += isr_numout(rep,      (double)sa  /smax,  k); break;
      case 'Y': n += isr_numout(rep, 100*((double)sa  /smax), k); break;
      case 'z': n += isr_numout(rep,      (double)sb  /smax,  k); break;
      case 'Z': n += isr_numout(rep, 100*((double)sb  /smax), k); break;
      case 's': n += isr_numout(rep,      (double)sab /smax,  k); break;
      case 'S': n += isr_numout(rep, 100*((double)sab /smax), k); break;
      case 't': supp = sa-sab;
                n += isr_numout(rep,      (double)supp/smax,  k); break;
      case 'T': supp = sa-sab;
                n += isr_numout(rep, 100*((double)supp/smax), k); break;
      case 'u': supp = sb-sab;
                n += isr_numout(rep,      (double)supp/smax,  k); break;
      case 'U': supp = sb-sab;
                n += isr_numout(rep, 100*((double)supp/smax), k); break;
      case 'c': conf = (body > 0) ? (double)sa  /(double)body : 0;
                n += isr_numout(rep,      conf,               k); break;
      case 'C': conf = (body > 0) ? (double)sa  /(double)body : 0;
                n += isr_numout(rep, 100* conf,               k); break;
      case 'd': supp = body -sa;
                conf = (body > 0) ? (double)supp/(double)body : 0;
                n += isr_numout(rep,      conf,               k); break;
      case 'D': supp = body -sa;
                conf = (body > 0) ? (double)supp/(double)body : 0;
                n += isr_numout(rep, 100* conf,               k); break;
      case 'e': conf = (body > 0) ? (double)sb  /(double)body : 0;
                n += isr_numout(rep,      conf,               k); break;
      case 'E': conf = (body > 0) ? (double)sb  /(double)body : 0;
                n += isr_numout(rep, 100* conf,               k); break;
      case 'f': supp = body -sb;
